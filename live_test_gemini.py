"""
라이브 테스트 #3: Gemini web_search 모드.

비용 예상: 약 $0.005 ~ $0.02 (gemini-3.1-pro-preview + google_search)
실행:
  python live_test_gemini.py
"""
import os
import sys
...
if not (os.environ.get("GEMINI_API_KEY", "").strip() or os.environ.get("GOOGLE_API_KEY", "").strip()):
    print("ERROR: GEMINI_API_KEY 또는 GOOGLE_API_KEY 비어있음")
    sys.exit(1)

print("=" * 70)
print("라이브 테스트 #3: Gemini web_search")
print("=" * 70)

from src.research_v2.registry import get_adapter

adapter = get_adapter("gemini", mode="web_search")
